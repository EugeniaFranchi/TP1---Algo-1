...
def life_crear(mapa):
    ...
    for fila in mapa:
        ...
        for caracter in fila:
            lista_booleana.append(caracter=="#")
        mapa_booleano.append(lista_booleana)
    return(mapa_booleano)
...
def cant_adyacentes(life, f, c):
    ...
    filas=[(f-1)%len(life),f%len(life),(f+1)%len(life)]
    columnas=[(c-1)%len(life[0]),c%len(life[0]),(c+1)%len(life[0])]
    
    for nro_de_fila in filas:
        for nro_de_celda in columnas:
            if nro_de_fila==f and nro_de_celda==c:
                continue
            if (life[nro_de_fila])[nro_de_celda]==True:
                contador+=1
    return contador
...
def celda_siguiente(life, f, c):
    ...
    if celda==False and n==3:
        return True
    if celda==True and (n==2 or n==3):
        return True
    return False
